...
public class OpenExchangeCurrencyConverterController extends ControllerCache {
	...
	public String showLatestRate(Model model, @Valid LatestRateModel latestRateModel, BindingResult bindingResult,
		...
		try {
			CurrencyRateModel currencyRateModel = openExchangeConverter.getLatestCurrencyRates(
					latestRateModel.getBaseCurrency(), latestRateModel.getAmount());
	...
	public String showHistoricalRate(Model model, @Valid HistoricalRateModel historicalRateModel, BindingResult bindingResult,
		...
		try {
			...
			List<RateModel> rateModels = getCurrencyRateModels(currencyRateModel, historicalRateModel.getHistoricalCurrency());
	...
	private void addExchangeHistory(Model model) {
		...
		Page<CurrencyHistory> currencyHistories = historyRepository.findByUserName(userName, new PageRequest(0, 10, Direction.DESC, "id"));
